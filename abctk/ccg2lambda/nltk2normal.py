...
def get_normal_form(expression):
    res = remove_true(expression)
    res = rename_variable(res)
    _counter.reset()
    return res
...
def is_wff(expression):
    # Check if a lambda term occurs in an expression
    if isinstance(expression, str):
        res = False
    elif isinstance(expression, ApplicationExpression):
        res = is_wff(expression.function) and is_wff(expression.argument)
    elif isinstance(expression, EqualityExpression):
        res = is_wff(expression.first) and is_wff(expression.second)
    elif isinstance(expression, AndExpression):
        res = is_wff(expression.first) and is_wff(expression.second)
    elif isinstance(expression, OrExpression):
        res = is_wff(expression.first) and is_wff(expression.second)
