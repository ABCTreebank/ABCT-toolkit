...
def coq_string_expr(expression):
    ...
    if isinstance(expression, ApplicationExpression):
        ...
        if str(fun) == 'pi':
            expr_coq_str = coq_string_pi_expr(expression)
        else:
...
def coq_string_pi_expr(expression):
    var_type = expression.function.argument
    body = expression.argument.term
    var = expression.argument.variable
    type_str = coqstr(var_type)
    body_str = coqstr(body)
    var_str = str(var)
    return Tokens.OPEN + 'forall ' + var_str + ':' + type_str + ',' + body_str + Tokens.CLOSE
