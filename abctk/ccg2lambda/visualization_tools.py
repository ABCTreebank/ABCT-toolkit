...
from .nltk2normal import get_normal_form
from .nltk2drs import convert_to_drs
from .drs_mathml import convert_to_drs_mathml
...
def get_semantics_mathml(semantics):
    return "<mtext " \
           ...
           + (html.escape(semantics) if semantics else "") \
           + "</mtext>\n"
...
def convert_sentence_to_mathml(sentence, drt: bool = False):
    mathml_str = ""
    
    gold_tree_index = int(sentence.get('gold_tree', -1))
    sentence_label = sentence.attrib['abc_id']
    ...
    if sem_trees and sem_trees[0].get('status') == 'success':
        formula_str = sem_trees[0].xpath('span[1]/@sem')[0]
        try:
...
def wrap_mathml_in_html(mathml_str):
    ...
    <head>
      <meta charset='UTF-8'/>
      <title>ABC to Semantic Representation</title>
...
def convert_root_to_mathml(root, drt=False):
    """
    ...
    for doc_ind, doc in enumerate(root.xpath('./document')):
        ...
        doc_mathml_str = convert_doc_to_mathml(doc, drt)
