...
from nltk.sem.logic import AbstractVariableExpression, ApplicationExpression, LogicalExpressionException
...
def get_combination_op(ccg_tree):
    ...
    """
    rule: str = ccg_tree.get('rule')
    if rule.startswith("|"):
        return "function_application"
    elif 'B' in rule or (len(rule) > 1 and (rule[1] not in ['a', 'p'])):
...
def _fold(
    pred: lg.Expression,
    args: typing.Iterable[lg.Expression]
) -> lg.Expression:
    pointer = pred
    for a in args:
        pointer = lg.ApplicationExpression(
            function = pointer,
            argument = a,
        )
    return pointer

def lambda_expansion(expr: lg.Expression):
    """
    In a post-hoc way, expand the special two-place predicate `lam`
    into a lambda expression.
    """
    # import sys
    # sys.stderr.write(f"expr: {expr}\n")
    ...
    if isinstance(expr, ApplicationExpression):
        ...
        # if "lam" in out:
        if (
            isinstance(pred, lg.AbstractVariableExpression) 
            and pred.variable.name == "lam"
        ):
            args_len = len(args)
            if args_len == 3:
                # sys.stderr.write(f"expr: {expr}, pred: {pred.variable}\n")
                var: AbstractVariableExpression
                var, term, arg = args
                return lg.ApplicationExpression(
                    function = lg.LambdaExpression(
                        variable = var.variable,
                        term = lambda_expansion(term),
                    ),
                    argument = lambda_expansion(arg)
                )
            elif args_len == 2:
                var: AbstractVariableExpression
                var, term = args
                return lg.LambdaExpression(
                    variable = var.variable,
                    term = lambda_expansion(term),
                )
            else:
                raise ValueError(
                    f"Wrong number of arguments ({args_len}) for 'lem'"
                )
        else:
            return _fold(
                pred = lambda_expansion(pred),
                args = (lambda_expansion(a) for a in args)
            )
    elif isinstance(expr, lg.LambdaExpression):
        return lg.LambdaExpression(
            variable = expr.variable,
            term = lambda_expansion(expr.term),
        )
    else:
        return expr
...
def assign_semantics(ccg_tree, semantic_index, tokens):
    ...
    if len(ccg_tree) == 0:
        # 0. fouod a lexical node
        semantics = semantic_index.get_semantic_representation(ccg_tree, tokens)
        ccg_tree.set('sem', str(semantics))
        return
    if len(ccg_tree) == 1:
        # 1. found a terminal (pre-lexical) or a unary node
        ...
        semantics = semantic_index.get_semantic_representation(ccg_tree, tokens)
        if semantics:
            semantics = lambda_expansion(semantics)
            ccg_tree.set('sem', str(semantics))
        else:
            pass
