...
class SemanticIndex(object):
    ...
    def get_semantic_representation(self, ccg_tree, tokens) -> typing.Optional[Expression]:
...
def build_default_template(rule_pattern, ccg_tree):
    ...
    if len(ccg_tree) == 0:
        ...
        if hasattr(ccg_tree, "get"):
            node_id = ccg_tree.get("id", "UNKNOWN")
        else:
            node_id = "UNKNOWN"
        raise ValueError(f"The node {node_id}, expected to be a terminal node, has more than one child.")
